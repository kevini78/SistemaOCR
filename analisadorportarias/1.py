...
class PortariaAnalyzer:
    ...
    def extrair_pessoas(self, texto):
        ...
        # Padrões para extrair informações - versões mais flexíveis
        patterns = [
            ...
            # Padrão para casos onde não há hífen, mas há vírgula
            r'([A-ZÀ-Ú][A-ZÀ-Ú\s]+?)\s*,\s*([A-Z]\d+[-]\w+|Processo\s+[\d\.\/]+)\s*,?\s*natural\s+d[oa]\s+([A-ZÀ-Ú\s]+)',
        ]
        ...
        for i, pattern in enumerate(patterns):
            ...
            if matches:
                print(f"✅ Encontradas {len(matches)} pessoas com padrão {i+1}")
        ...
        # Limpar pessoas com nomes inválidos (a verificação de duplicatas será feita em verificar_erros)
        pessoas_limpas = []
    ...
    def extrair_dados_portaria_direto(self, texto_portaria):
        ...
        # Extrair número e data da portaria - padrões mais flexíveis
        patterns_portaria = [
            r'PORTARIA\s*,?\s*[NnNº°]?\s*(\d+[\.,]?\d*)\s*,?\s*DE\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',
            r'PORTARIA\s*N[º°]?\s*(\d+[\.,]?\d*)[,\s]*DE\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',
            r'PORTARIA\s*(\d+[\.,]?\d*)[,\s]*DE\s*(\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})',
            r'Portaria\s*n[º°]?\s*(\d+[\.,]?\d*)[,\s]*de\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})',
            # Padrão mais simples para casos onde não há "DE"
            r'PORTARIA\s*[NnNº°]?\s*(\d+[\.,]?\d*)',
        ]
    ...
    def analisar_multiplas_portarias(self, texto_completo, gerar_excel=True):
        ...
        """
        # Regex para identificar o início de uma portaria de naturalização real,
        # procurando por "PORTARIA Nº [numero], DE [data]".
        # Usa lookahead (?=...) para dividir o texto MANTENDO o delimitador no início de cada bloco.
        padrao_delimitador = r'(?=PORTARIA\s+N[º°]?\s*[\d\.,]+\s*,\s*DE\s+\d{1,2}\s+DE\s+\w+\s+DE\s+\d{4})'

        # Dividir o texto em blocos de portaria
        blocos_candidatos = re.split(padrao_delimitador, texto_completo, flags=re.IGNORECASE)

        # Filtrar blocos que são de fato portarias de naturalização
        blocos = []
        for bloco_cand in blocos_candidatos:
            bloco_strip = bloco_cand.strip()
            # Garantir que o bloco comece com 'PORTARIA N' para ser válido
            if bloco_strip and re.match(r'PORTARIA\s+N[º°]?', bloco_strip, re.IGNORECASE):
                blocos.append(bloco_strip)
